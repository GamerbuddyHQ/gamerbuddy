...
const SKILL_CONFIG: Record<string, { border: string; text: string; bg: string; glow: string; bar: string }> = {
  ...
  Expert:       { border: "border-primary/40",    text: "text-primary",    bg: "bg-primary/10",    glow: "rgba(255,77,45,0.2)",    bar: "#FF4D2D" },
  Chill:        { border: "border-cyan-500/40",   text: "text-cyan-400",   bg: "bg-cyan-500/10",   glow: "rgba(255,77,45,0.15)",   bar: "#FF4D2D" },
};
const DEFAULT_SKILL = { border: "border-primary/30", text: "text-primary", bg: "bg-primary/10", glow: "rgba(255,77,45,0.15)", bar: "#FF4D2D" };
...
function QuickBidPanel({ req, onClose }: { req: GameRequest; onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit}
      ...
      style={{ boxShadow: "0 0 24px rgba(255,77,45,0.07) inset" }}
    >
      ...
      <div className="flex flex-wrap gap-2.5 pt-1">
        ...
        <Button
          ...
          className="bg-primary hover:bg-primary/90 flex-1 min-w-[140px] font-bold uppercase tracking-wider text-xs shadow-[0_0_20px_rgba(255,77,45,0.3)] disabled:opacity-50 h-10"
        >
          {state === "submitting" ? (
  ...
  return (
    <div
      className={`group rounded-2xl border overflow-hidden transition-[transform,border-color,box-shadow] duration-300 will-change-transform ${
        expanded
          ? "border-primary/55 shadow-[0_12px_64px_rgba(255,77,45,0.28)] translate-y-0"
          : "border-border/35 hover:border-primary/50 hover:-translate-y-[6px] hover:shadow-[0_20px_64px_rgba(255,77,45,0.22)]"
      }`}
      style={{
        background: isDark
          ? expanded
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {req.lowestBid && (
                  <div
                    ...
                    style={{ borderColor: "rgba(255,77,45,0.22)", background: "rgba(255,77,45,0.06)" }}
                  >
              ...
              <div className="relative flex-1 sm:flex-none">
                {!expanded && (
                  <div
                    ...
                    style={{ background: "linear-gradient(135deg, #FF4D2D, #cc3a1e)", filter: "blur(16px)" }}
                  />
                )}
                <button
                  ...
                  style={
                    !expanded
                      ? { background: "linear-gradient(135deg, #FF4D2D 0%, #cc3a1e 100%)", boxShadow: "0 6px 40px rgba(255,77,45,0.65), inset 0 1px 0 rgba(255,255,255,0.18)" }
                      : {}
                  }
                >
  ...
  return (
    <div className="space-y-6">
      <div
        ...
        style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(255,77,45,0.04) 0%, transparent 70%)" }}
      >
        ...
        <div className="flex items-center justify-center gap-3 mt-6">
          ...
          <Button size="sm" className="bg-primary text-white text-xs shadow-[0_0_16px_rgba(255,77,45,0.3)]" onClick={() => setLocation("/post-request")}>
            Post a Request
          </Button>
        </div>
      </div>
...
function HowItWorksSection() {
  const { isDark } = useTheme();
  const steps = [
    { num: "01", icon: <Search className="h-5 w-5" />, color: "#FF4D2D", title: "Verify & Activate",  desc: "Link a gaming account → 24–48hr review → pay a one-time activation fee (🇮🇳 ₹149 / 🌍 $5) — then you're fully unlocked." },
    { num: "02", icon: <Gavel  className="h-5 w-5" />, color: "#FF4D2D", title: "Browse & Bid",       desc: "Find requests that match your skills and platform. Set your price and pitch yourself — no middlemen." },
  ...
  return (
    <div
      ...
      style={{
        borderColor: isDark ? "rgba(255,77,45,0.18)" : "rgba(255,77,45,0.20)",
        background: isDark
          ? "linear-gradient(135deg, rgba(255,77,45,0.05) 0%, rgba(255,77,45,0.03) 100%)"
          : "linear-gradient(135deg, rgba(255,77,45,0.04) 0%, rgba(255,77,45,0.02) 100%)",
        boxShadow: isDark
          ? "0 0 40px rgba(255,77,45,0.08), 0 0 80px rgba(255,77,45,0.04)"
          : "0 0 32px rgba(255,77,45,0.07), 0 2px 16px rgba(0,0,0,0.04)",
      }}
    >
      {/* Heading */}
      <div>
    ...
    <div className="space-y-6 pt-4">
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-2.5 pb-2">
        {[
          ...
          { icon: <Target className="h-3.5 w-3.5" style={{ color: "#FF4D2D" }} />, text: "Escrow-secured payments" },
        ].map((t) => (
...
const LEVEL_OPTIONS = [
  ...
  { value: "Expert",       label: "Best / Expert",     dot: "#FF4D2D" },
  { value: "Chill",        label: "Chill",             dot: "#FF4D2D" },
];
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse 60% 40% at 70% 10%, rgba(255,77,45,0.08) 0%, transparent 60%), radial-gradient(ellipse 40% 30% at 20% 80%, rgba(255,77,45,0.04) 0%, transparent 50%)",
        }}
      />
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-[11px] font-semibold" style={{ color: "rgba(255,77,45,0.55)" }}>
        ...
        {!isLoading && requests !== undefined && (
          <div className="flex items-center gap-2 shrink-0 self-start sm:self-auto">
            <div
              ...
              style={{
                background: isDark
                  ? "linear-gradient(135deg, rgba(255,77,45,0.1) 0%, rgba(0,0,0,0.3) 100%)"
                  : "linear-gradient(135deg, rgba(255,77,45,0.10) 0%, rgba(255,77,45,0.04) 100%)",
      ...
      <div
        ...
        style={{
          borderColor: hasFilters ? "rgba(255,77,45,0.40)" : isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.10)",
          background: isDark ? "rgba(7,5,16,0.96)" : "hsl(var(--card))",
          backdropFilter: "blur(24px)",
          WebkitBackdropFilter: "blur(24px)",
          boxShadow: hasFilters
            ? "0 6px 32px rgba(0,0,0,0.20), 0 0 0 1px rgba(255,77,45,0.10)"
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            background: hasFilters ? "rgba(255,77,45,0.07)" : isDark ? "rgba(255,255,255,0.025)" : "rgba(0,0,0,0.02)",
            transition: "background 0.3s",
          }}
        >
          <div className="flex items-center gap-2.5">
            <SlidersHorizontal className="h-4 w-4" style={{ color: hasFilters ? "#FF4D2D" : isDark ? "rgba(255,255,255,0.35)" : "rgba(0,0,0,0.40)" }} />
            ...
            {hasFilters && (
              <span
                ...
                style={{ background: "rgba(255,77,45,0.25)", color: "#FF4D2D", border: "1px solid rgba(255,77,45,0.40)" }}
              >
            ...
            {SORT_OPTIONS.map(({ value, label, Icon }) => {
              ...
              return (
                <button
                  ...
                  style={active ? {
                    background: "rgba(255,77,45,0.20)",
                    border: "1px solid rgba(255,77,45,0.50)",
                    color: "#FF4D2D",
                    boxShadow: "0 0 16px rgba(255,77,45,0.22)",
                  } : {
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {([
              ...
              { label: "Easy Wins",       Icon: Flame,  on: noBidsOnly,         set: () => setNoBidsOnly(v => !v),         activeStyle: { bg: "rgba(255,77,45,0.15)", border: "rgba(255,77,45,0.45)", color: "#FF4D2D", glow: "rgba(255,77,45,0.14)" } },
        ...
        {hasFilters && (
          <div
            className="border-t"
            style={{
              borderColor: "rgba(255,77,45,0.18)",
              background: "linear-gradient(180deg,rgba(255,77,45,0.06) 0%,rgba(255,77,45,0.02) 100%)",
            }}
          >
            ...
            <div className="flex items-center gap-2 px-6 pt-4 pb-2.5">
              ...
              <span
                ...
                style={{ color: "rgba(255,77,45,0.75)" }}
              >
                Active Filters
              </span>
              <span
                className="text-[9px] font-black px-1.5 py-0.5 rounded-full shrink-0"
                style={{ background: "rgba(255,77,45,0.22)", color: "#FF4D2D", border: "1px solid rgba(255,77,45,0.38)" }}
              >
                {activeTags.length}
              </span>
              {/* Divider line */}
              <span className="flex-1 h-px mx-1" style={{ background: "rgba(255,77,45,0.12)" }} />
              {/* Result count */}
              <span className="text-[10px] font-semibold shrink-0 tabular-nums" style={{ color: isDark ? "rgba(255,255,255,0.35)" : "rgba(0,0,0,0.45)" }}>
                {requests?.length === allRequests?.length
                  ...
                  : (
                    <>
                      <span style={{ color: isDark ? "rgba(255,77,45,0.80)" : "hsl(228 60% 45%)" }}>{requests?.length ?? 0}</span>
