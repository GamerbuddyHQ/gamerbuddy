...
const TYPE_CONFIG: Record<TournamentType, { label: string; desc: string; icon: React.FC<{ className?: string }>; color: string; defaultPlayers: number }> = {
  ...
  squad: { label: "Squad Battle",  desc: "Team-based clash",           icon: Users,   color: "#DCCE40", defaultPlayers: 4 },
    ...
    <span className="relative group inline-flex items-center ml-1.5 align-middle cursor-help">
      ...
      <span
        ...
        style={{
          ...
          boxShadow: "0 8px 24px rgba(0,0,0,0.6), 0 0 0 1px rgba(220,206,64,0.10)",
        }}
      >
        {text}
        <span className="absolute top-full left-1/2 -translate-x-1/2 -mt-px border-4 border-transparent" style={{ borderTopColor: "rgba(220,206,64,0.35)" }} />
      </span>
    </span>
  );
}
...
function ChipRow({ chips, active, onSelect, color = "purple" }: {
  ...
  const activeStyle = color === "gold"
    ...
    : { background: "rgba(220,206,64,0.22)", border: "1px solid rgba(220,206,64,0.55)", color: "#DCCE40" };
...
function ReqBadge({ country, region, gender }: { country: string; region: string; gender: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap mt-2">
      ...
      {hasGender && (
        <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-md"
          style={{ background: "rgba(220,206,64,0.10)", border: "1px solid rgba(220,206,64,0.25)", color: "#DCCE40" }}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                ...
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md text-primary/80" style={{ background: "rgba(220,206,64,0.10)", border: "1px solid rgba(220,206,64,0.22)" }}>
                  {tournament.gameName}
                </span>
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] text-muted-foreground/50 font-medium flex items-center gap-1.5">
              ...
              {isOpen && !isFull
                ? <><span className="font-extrabold" style={{ color: pct > 60 ? "#fbbf24" : "#DCCE40" }}>{tournament.slotsLeft}</span> slots left</>
                : isFull ? <span className="font-bold text-red-400/80">All slots filled</span>
                : "Participants"}
            </span>
            <span className="text-[12px] font-extrabold tabular-nums" style={{ color: pct >= 100 ? "#f87171" : pct > 60 ? "#fbbf24" : "#DCCE40" }}>
          ...
          <div className="h-2.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
            <div className="h-full rounded-full transition-all duration-700" style={{
              width: `${pct}%`,
              background: pct >= 100 ? "linear-gradient(90deg,#f87171,#ef4444)" : pct > 75 ? "linear-gradient(90deg,#f97316,#ea580c)" : pct > 40 ? "linear-gradient(90deg,#fbbf24,#f59e0b)" : "linear-gradient(90deg,#DCCE40,#b5a730)",
            }} />
          </div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap pt-0.5">
          ...
          {isOpen && !isHost && !myReg && currentUserId && (
            eligibility.eligible ? (
              <Button
                ...
                style={{ background: "linear-gradient(135deg,#DCCE40,#b5a730)", boxShadow: "0 0 20px rgba(220,206,64,0.30)" }}
              >
          ...
          {!currentUserId && isOpen && (
            <Button asChild className="flex-1 font-bold py-2.5 h-auto" style={{ background: "linear-gradient(135deg,#DCCE40,#b5a730)" }}>
          ...
          {isHost && (
            <span className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-[12px] font-bold"
              style={{ background: "rgba(220,206,64,0.10)", border: "1px solid rgba(220,206,64,0.28)", color: "#DCCE40" }}>
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border"
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b"
          ...
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl flex items-center justify-center"
              style={{ background: "linear-gradient(135deg,rgba(220,206,64,0.30),rgba(220,206,64,0.12))", border: "1px solid rgba(220,206,64,0.50)" }}>
        ...
        <div className="p-6 space-y-6">
          ...
          <section>
            ...
            <div className="space-y-3 p-4 rounded-xl" style={{ background: "rgba(220,206,64,0.04)", border: "1px solid rgba(220,206,64,0.14)" }}>
              <div className="grid grid-cols-2 gap-3">
                <div>
          ...
          <section>
              ...
              {DIST_PRESETS.map((p) => {
                ...
                return (
                  <button key={p.key} type="button" onClick={() => setDistPreset(p.key)}
                    ...
                    style={active ? { background: "rgba(220,206,64,0.16)", border: "1.5px solid rgba(220,206,64,0.55)" }
                      : { background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
                    <p className="text-[11px] font-extrabold" style={{ color: active ? "#DCCE40" : "rgba(255,255,255,0.45)" }}>{p.label}</p>
                    {p.key !== "custom" && (
                    ...
                    {p.key !== "custom"
                      ? <p className="text-[9px]" style={{ color: active ? "rgba(220,206,64,0.65)" : "rgba(255,255,255,0.22)" }}>
                          {[p.first > 0 && `🥇${p.first}%`, p.second > 0 && `🥈${p.second}%`, p.third > 0 && `🥉${p.third}%`].filter(Boolean).join(" ")}
                        </p>
                      : <p className="text-[9px]" style={{ color: active ? "rgba(220,206,64,0.65)" : "rgba(255,255,255,0.22)" }}>set your own %</p>}
                  </button>
                );
              })}
            </div>
            ...
            {distPreset === "custom" && (
              <div className="mt-3 p-4 rounded-xl space-y-3" style={{ background: "rgba(220,206,64,0.05)", border: "1px solid rgba(220,206,64,0.15)" }}>
                {[
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="rounded-3xl p-6 sm:p-8 relative overflow-hidden"
        style={{ background: "linear-gradient(135deg,rgba(220,206,64,0.12) 0%,rgba(251,191,36,0.08) 50%,rgba(0,0,0,0.5) 100%)", border: "1px solid rgba(220,206,64,0.25)", boxShadow: "0 0 50px rgba(220,206,64,0.10)" }}>
          ...
          <div className="flex items-center p-1 gap-1 rounded-xl shrink-0"
            ...
            {FILTERS.map(({ value, label, count }) => (
              <button key={value} onClick={() => setFilter(value)}
                ...
                style={filter === value ? { background: "linear-gradient(135deg,rgba(220,206,64,0.28),rgba(220,206,64,0.18))", border: "1px solid rgba(220,206,64,0.55)", color: "#DCCE40" }
                  : { color: "rgba(255,255,255,0.40)", border: "1px solid transparent" }}>
                {label}
                {count !== undefined && count > 0 && (
                  <span className="text-[9px] font-black px-1 rounded-full"
                    style={filter === value ? { background: "rgba(220,206,64,0.35)", color: "#e9d5ff" } : { background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.35)" }}>
                    {count}
                  </span>
                )}
              </button>
        ...
        <div className="flex items-center gap-2">
          ...
          {SORT_OPTIONS.map((o) => (
            <button key={o.value} onClick={() => setSort(o.value)}
              ...
              style={sort === o.value ? { background: "rgba(220,206,64,0.18)", border: "1px solid rgba(220,206,64,0.40)", color: "#DCCE40" }
