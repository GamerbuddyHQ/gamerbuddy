...
const TYPE_META: Record<string, { label: string; color: string }> = {
  session_fee:      { label: "Session Fee",     color: "#00D4FF" },
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="rounded-xl border border-border/60 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-4 border-b border-border/40"
              style={{ background: "rgba(0,212,255,0.06)", borderColor: "rgba(0,212,255,0.15)" }}>
              <div>
                <p className="text-sm font-bold" style={{ color: "#00D4FF" }}>You Keep (10% Platform Fee)</p>
              ...
              <span className="text-2xl font-black tabular-nums" style={{ color: "#00D4FF" }}>
                {fmtUSD(fee.amount)}
              </span>
            </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-6 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="rounded-2xl border flex items-center gap-4 px-6 py-4"
            style={{ borderColor: "rgba(0,212,255,0.30)", background: "rgba(0,212,255,0.06)" }}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: "rgba(0,212,255,0.70)" }}>You Keep (All Time)</p>
              <p className="text-3xl font-black tabular-nums leading-tight" style={{ color: "#00D4FF" }}>{fmtUSD(totalAllTimeKeep)}</p>
              <p className="text-[11px] mt-0.5" style={{ color: "rgba(0,212,255,0.50)" }}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                      ...
                      <div className="flex items-center gap-2 shrink-0 flex-wrap sm:flex-nowrap">
                        ...
                        <div className="rounded-xl border px-3 py-2 text-center min-w-[72px]"
                          ...
                          <p className="text-[9px] font-black uppercase tracking-wider" style={{ color: "rgba(0,212,255,0.65)" }}>You Keep</p>
                          <p className="text-sm font-bold tabular-nums" style={{ color: "#00D4FF" }}>{fmtUSD(fee.amount)}</p>
                        </div>
                        <span className="text-muted-foreground/25 text-lg font-light">→</span>
                        {/* You Send */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl border px-6 py-6 flex items-center gap-5"
            ...
            <div className="text-5xl font-black tabular-nums leading-none shrink-0" style={{ color: "#00D4FF" }}>10%</div>
            <div>
              <p className="text-base font-extrabold uppercase tracking-wide" style={{ color: "#00D4FF" }}>You Keep</p>
              <p className="text-xs mt-1 leading-relaxed" style={{ color: "rgba(0,212,255,0.55)" }}>
                Platform fee goes to <strong className="font-mono" style={{ color: "rgba(0,212,255,0.8)" }}>creedx112@okicici</strong> via Razorpay. Same split applies to all tips and bulk sessions.
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              { label: "This Week",  value: fmtUSD(d.weekFees),   sub: `🇮🇳 India: ${fmtUSD(d.india.total)}`,      icon: <CalendarDays className="h-5 w-5" />,  color: "#22d3ee" },
              { label: "This Month", value: fmtUSD(d.monthFees),  sub: `🌍 Global: ${fmtUSD(d.global.total)}`,    icon: <CalendarRange className="h-5 w-5" />, color: "#00D4FF" },
            ].map((card) => (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Total All-Time", value: fmtUSD(d.totalFees),   color: "#00D4FF", icon: <DollarSign className="h-4 w-4" /> },
            { label: "Session Fees",   value: fmtUSD(d.sessionFees), color: "#22d3ee", icon: <ShieldCheck className="h-4 w-4" /> },
              ...
              <table className="w-full text-xs min-w-[820px]">
                <thead>
                  <tr className="border-b border-border/20 bg-muted/10">
                    ...
                    <th className="text-right px-4 py-3 font-bold uppercase tracking-widest whitespace-nowrap" style={{ color: "rgba(0,212,255,0.7)" }}>You Keep (10%)</th>
                ...
                <tbody className="divide-y divide-border/15">
                  {visibleFees.map((fee, idx) => {
                    const meta = TYPE_META[fee.type] ?? { label: fee.type, color: "#00D4FF" };
                    const isIndia = fee.hirerRegion === "india";
                    ...
                    return (
                      <tr key={fee.id} onClick={() => setModalFee(fee)}
                        ...
                        <td className="px-4 py-3 text-right whitespace-nowrap">
                          <span className="font-bold tabular-nums" style={{ color: "#00D4FF" }}>{fmtUSD(fee.amount)}</span>
                        </td>
                ...
                <tfoot>
                  <tr className="border-t-2 border-border/40 bg-muted/30">
                    ...
                    <td className="px-4 py-4 text-right whitespace-nowrap">
                      <div className="text-[9px] uppercase tracking-widest font-bold" style={{ color: "rgba(0,212,255,0.55)" }}>You Keep</div>
                      <div className="text-base font-black tabular-nums" style={{ color: "#00D4FF" }}>{fmtUSD(totalKeep)}</div>
                    </td>
