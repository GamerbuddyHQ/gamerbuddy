...
function WalletPill({ wallet }: { wallet: string }) {
  const color = wallet === "hiring" ? "#4F9EFF" : "#38BAFF";
  return (
    <span
    ...
    <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-8 space-y-8">
      ...
      {isLoading ? (
        ...
        <>
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard icon={Users}      label="Total Users"         value={data.users.total}    sub="All registered accounts"   color="#38BAFF" />
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatCard icon={TrendingUp}    label="Large Txns"      value={largeTxCount} sub="Transactions ≥ $500"     color="#4F9EFF" />
            </div>
          </div>
        </>
      ) : null}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          >
            <DollarSign className="h-5 w-5" style={{ color: "#4F9EFF" }} />
          </div>
          <div>
        ...
        <a
          ...
          style={{ borderColor: "rgba(79,158,255,0.40)", color: "#4F9EFF", background: "rgba(79,158,255,0.08)" }}
        >
          View Earnings →
        </a>
      </div>
