...
function renderMarkdown(text: string) {
  ...
  return lines.map((line, i) => {
    const processed = line
      ...
      .replace(/`(.*?)`/g, "<code style='font-size:11px;background:rgba(200,240,0,0.15);padding:1px 4px;border-radius:4px'>$1</code>");
    return (
      <span key={i}>
  ...
  const [messages, setMessages]   = useState<Message[]>([
    {
      id: 0,
      role: "assistant",
      text: "Yo! 👾 I'm **Buddy** — your Player4Hire support assistant, always online.\n\nWe're in **Phase 1** — the full core experience is live! To get started: sign up → link a gaming account → get verified (24–48hrs) → pay the one-time activation fee (🇮🇳 ₹149 / 🌍 $5) → start hiring or bidding!\n\nAsk me anything — wallets, escrow, fees, how to post a request, how to bid, safety rules, and more. Let's get you that W! 🎮",
    },
  ]);
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-5 right-5 z-50 flex items-center gap-2.5 rounded-full px-4 py-3 font-bold text-sm text-[#C8F000] transition-all hover:scale-105 active:scale-95"
        style={{ background: "#1A1A1A", boxShadow: "0 4px 12px rgba(0,0,0,0.50)", border: "1px solid rgba(200,240,0,0.20)" }}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: "0 0 40px rgba(200,240,0,0.18), 0 8px 32px rgba(0,0,0,0.6)",
      }}
    >
      {/* Header */}
      <div
        className="flex items-center justify-between px-4 py-3 shrink-0"
        style={{ background: "linear-gradient(90deg, rgba(200,240,0,0.6) 0%, rgba(200,240,0,0.4) 100%)", borderBottom: "1px solid rgba(200,240,0,0.2)" }}
      >
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto px-3.5 py-3 space-y-3 scrollbar-thin">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-2 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                <div
                  ...
                  style={
                    msg.role === "assistant"
                      ? { background: "rgba(200,240,0,0.15)", border: "1px solid rgba(200,240,0,0.3)" }
                      : { background: "rgba(200,240,0,0.15)", border: "1px solid rgba(200,240,0,0.3)" }
                  }
                >
                  {msg.role === "assistant"
                ...
                <div
                  ...
                  style={
                    msg.role === "assistant"
                      ? { background: "rgba(200,240,0,0.08)", border: "1px solid rgba(200,240,0,0.15)", color: "rgba(255,255,255,0.88)" }
                      : { background: "rgba(200,240,0,0.12)", border: "1px solid rgba(200,240,0,0.2)", color: "rgba(255,255,255,0.92)" }
                  }
                >
            ...
            {typing && (
              <div className="flex gap-2">
                <div className="shrink-0 h-7 w-7 rounded-full flex items-center justify-center" style={{ background: "rgba(200,240,0,0.15)", border: "1px solid rgba(200,240,0,0.3)" }}>
                  <Bot className="h-3.5 w-3.5 text-primary" />
                </div>
                <div className="rounded-2xl px-4 py-3" style={{ background: "rgba(200,240,0,0.08)", border: "1px solid rgba(200,240,0,0.15)" }}>
          ...
          <div className="px-3.5 pb-2 shrink-0">
            <a
              ...
              style={{
                background: "linear-gradient(135deg, #C8F000 0%, #C5A46E 60%, #C8F000 100%)",
                boxShadow: "0 0 14px rgba(200,240,0,0.25)",
              }}
            >
          ...
          {/* Input */}
          <div className="px-3.5 pb-3.5 pt-2 shrink-0" style={{ borderTop: "1px solid rgba(200,240,0,0.12)" }}>
            <div
              className="flex items-center gap-2 rounded-xl px-3.5 py-2"
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(200,240,0,0.2)" }}
            >
              <input
              ...
              <button
                ...
                style={input.trim()
                  ? { background: "rgba(200,240,0,0.25)", border: "1px solid rgba(200,240,0,0.4)", color: "#C8F000" }
