...
const RESPONSES: Array<{ patterns: RegExp; reply: string }> = [
  ...
  {
    patterns: /minimum.*fee|min.*fee|minimum.*rate|min.*rate|minimum.*price|minimum.*pay|how much.*cost|what.*cost.*hire|pricing|how much.*hire|hire.*cost|per.hour|hourly.*rate|₹200|₹350|rupee|inr.*rate|india.*rate|international.*rate|\$5|\$8|5 usd|8 usd|objective.*price|fair pay/i,
    reply: `💰 **Minimum Bid — Objective-Based Pricing**\n\nPlayer4Hire is **objective-based** — no hourly billing. Hirers post a fixed budget for a specific objective, gamers bid on it, and the session ends when the objective is complete.\n\n🇮🇳 **India:** Minimum **₹200 per objective**\n🌍 **Global (all other countries):** Minimum **$5 USD per objective**\n\n**How it works:**\n• Post a request with your game, platform, and objective (e.g. "Carry me to Diamond in Apex")\n• Set your region — this locks in the minimum floor\n• Gamers bid a flat amount to complete your objective — no hours involved\n• The minimum floor is displayed clearly on every request listing\n\n**Why this model?**\nObjective-based pricing is fairer — you pay for results, not time. Gamers set a price they feel reflects the effort needed, and the session closes when the goal is achieved.\n\nYou can see the minimum bid on every listing on the **[Browse page](/browse)** and in the bid panel. 🎮`,
  },
