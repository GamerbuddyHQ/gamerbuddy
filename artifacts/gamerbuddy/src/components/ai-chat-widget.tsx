...
function renderMarkdown(text: string) {
  ...
  return lines.map((line, i) => {
    const processed = line
      ...
      .replace(/`(.*?)`/g, "<code style='font-size:11px;background:rgba(0,212,255,0.15);padding:1px 4px;border-radius:4px'>$1</code>");
    return (
      <span key={i}>
  ...
  if (!open) {
    return (
      <button
        ...
        style={{ background: "linear-gradient(135deg, #0099bb 0%, #00D4FF 50%, #0099bb 100%)", boxShadow: "0 0 24px rgba(0,212,255,0.45), 0 4px 20px rgba(0,0,0,0.4)" }}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: "0 0 40px rgba(0,212,255,0.18), 0 8px 32px rgba(0,0,0,0.6)",
      }}
    >
      {/* Header */}
      <div
        className="flex items-center justify-between px-4 py-3 shrink-0"
        style={{ background: "linear-gradient(90deg, rgba(0,212,255,0.6) 0%, rgba(0,212,255,0.4) 100%)", borderBottom: "1px solid rgba(0,212,255,0.2)" }}
      >
      ...
      {!minimized && (
        <>
          ...
          <div className="flex-1 overflow-y-auto px-3.5 py-3 space-y-3 scrollbar-thin">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-2 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div
                  ...
                  style={
                    msg.role === "assistant"
                      ? { background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.15)", color: "rgba(255,255,255,0.88)" }
            ...
            {typing && (
              <div className="flex gap-2">
                <div className="shrink-0 h-7 w-7 rounded-full flex items-center justify-center" style={{ background: "rgba(0,212,255,0.15)", border: "1px solid rgba(0,212,255,0.3)" }}>
                  <Bot className="h-3.5 w-3.5 text-primary" />
                </div>
                <div className="rounded-2xl px-4 py-3" style={{ background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.15)" }}>
          ...
          {/* Input */}
          <div className="px-3.5 pb-3.5 pt-2 shrink-0" style={{ borderTop: "1px solid rgba(0,212,255,0.12)" }}>
            <div
            ...
            >
              ...
              <button
                ...
                style={input.trim()
                  ? { background: "rgba(0,212,255,0.25)", border: "1px solid rgba(0,212,255,0.4)", color: "#00D4FF" }
